...
function ChangeComponent() {
  const { admin, brands, category } = useSelector((state) => state.todos);
  const { id } = useParams();
  const dispatch = useDispatch();
  const productDetails = useSelector((state) => state.todos.productDetail);
  let token = localStorage.getItem("token");
  token = JSON.parse(token);
  const msg = useSelector((state) => state.todos.productChangedMsg);
